...
class vect {
 ...
 public:
  ...
  inline friend vect operator*(double a,
                               vect b);  ///< multiply 4-vector by a number
  inline friend vect operator*(vect b,
                               double a);  ///< multiply 4-vector by a number
  inline friend vect operator/(vect b,
                               double a);  ///< divide 4-vector by a number
...
double& vect::operator[](int i) { return *(&t + i); }
...
vect vect::operator-() { return vect(-t, -x, -y, -z); }
...
vect::operator vec() const { return vec(x, y, z); }

vec vect::v() { return vec(x / t, y / t, z / t); }
...
{                          // cout<<v<<endl;
  ...
  *this = vect(t + x * v.x + y * v.y + z * v.z, x + t * v.x, y + t * v.y,
               z + t * v.z) *
              gamma +
          stale;
  //     double new_norm=*this * *this;
  //     cout<<"Niedokadność
  //     boostowania="<<old_norm-new_norm<<"/"<<old_norm<<endl;
  return *this;
}
...
double vect::length() { return sqrt(x * x + y * y + z * z); }
...
vect& vect::boost(vec v) { return boost1(v); }

///////////////////////////////////////////////////////////////////////////
///                  FUNKCJE POMOCNICZE
///////////////////////////////////////////////////////////////////////////

vec speed(vect a, vect b) {
  vect c = a + b;
  return c.v();
}

double q2(vect k, vect kprim) {
  vect q = k - kprim;
  return q * q;
}

double q0(vect k, vect kprim) { return k.t - kprim.t; }
