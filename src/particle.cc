...
bool decay(vect p4, particle &p1, particle &p2) {
  double MM = p4 * p4;
  double Msum = p1.mass() + p2.mass();
  if (Msum * Msum >= MM) return false;
...
double get_cos(double a, double b, double c, double d, double e, double f,
               double g, double h) {
  double max = fabs(a) + fabs(b) + fabs(c) + fabs(d) + fabs(e) + fabs(f) +
               fabs(g) + fabs(h);
  ...
  } while (a * x7 + b * x6 + c * x5 + d * x4 + e * x3 + f * x2 + g * x + h <
           max * frandom());
