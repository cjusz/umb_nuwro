...
void NuwroReWeight_BBBA07::Reconfigure(void){
  ...
  NuwroSyst_t curenums[7] = {kNuwro_BBBA07_AEp1, kNuwro_BBBA07_AEp2, kNuwro_BBBA07_AEp3, kNuwro_BBBA07_AEp4, kNuwro_BBBA07_AEp5, kNuwro_BBBA07_AEp6, kNuwro_BBBA07_AEp7};
  for (int i = 0; i < 7; i++){
    double fError = fracerr->OneSigmaErr(curenums[i], p_AEp_twk[i] > 0);
    p_AEp[i] = p_AEp_def[i] * (1 + fError * p_AEp_twk[i]);
  }

  NuwroSyst_t curenums2[7] = {kNuwro_BBBA07_AMp1, kNuwro_BBBA07_AMp2, kNuwro_BBBA07_AMp3, kNuwro_BBBA07_AMp4, kNuwro_BBBA07_AMp5, kNuwro_BBBA07_AMp6, kNuwro_BBBA07_AMp7};
  for (int i = 0; i < 7; i++){
    double fError = fracerr->OneSigmaErr(curenums2[i], p_AMp_twk[i] > 0);
    p_AMp[i] = p_AMp_def[i] * (1 + fError * p_AMp_twk[i]);
  }

  NuwroSyst_t curenums3[7] = {kNuwro_BBBA07_AEn1, kNuwro_BBBA07_AEn2, kNuwro_BBBA07_AEn3, kNuwro_BBBA07_AEn4, kNuwro_BBBA07_AEn5, kNuwro_BBBA07_AEn6, kNuwro_BBBA07_AEn7};
  for (int i = 0; i < 7; i++){
    double fError = fracerr->OneSigmaErr(curenums3[i], p_AEn_twk[i] > 0);
    p_AEn[i] = p_AEn_def[i] * (1 + fError * p_AEn_twk[i]);
  }

  NuwroSyst_t curenums4[7] = {kNuwro_BBBA07_AMn1, kNuwro_BBBA07_AMn2, kNuwro_BBBA07_AMn3, kNuwro_BBBA07_AMn4, kNuwro_BBBA07_AMn5, kNuwro_BBBA07_AMn6, kNuwro_BBBA07_AMn7};
  for (int i = 0; i < 7; i++){
