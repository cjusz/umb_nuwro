...
void DisplayInfo() {
  std::cout << std::endl;
  std::cout << "Usage: nuwro2rootracker [-x extended] [-n #evts] [-p POT] "
               "nuwrofile.root rootrackerfile_template.root"
            << std::endl;
  std::cout << "Converts nuwro output file to a series of rootracker tree "
               "format files."
            << std::endl;
  std::cout << "Extended mode splits up struck nucleon and nuclear target "
               "entries and also writes out pre-FSI info."
            << std::endl;
  std::cout
      << "The @ character in the template will be substituted with file number."
      << std::endl;
  std::cout << "#evts is the number of events per one output file" << std::endl;
  std::cout << "(must be less than the number of evts in the input file)."
            << std::endl;
  std::cout << "If #evts not specified, one output file will be created."
            << std::endl;
...
int main(int argc, char *argv[]) {
    ...
    switch (c) {
      ...
      case 'x': {
        std::cout << "[INFO]: Running in extended output mode. "
                     "Will output event::out vector to StdHep arrays"
                  << std::endl;
        extended_mode = true;
        break;
      }
  ...
  for (Long64_t ent = 0; ent < nEntries;) {
    ...
    for (; ent < std::min(NCopiedEvts + nEntriesToCopy, nEntries); ++ent) {
      ...
      for (size_t nin = 0; nin < nwEv->in.size(); ++nin) {
        ...
        if (in_part.pdg == 2112 ||
          ...
          if (nwEv->par.nucleus_p == 1 &&
              nwEv->par.nucleus_n == 0) {                  // hydrogen
      ...
      for (int npost = 0; npost < nwEv->post.size(); npost++) {
        ...
        rtEv.fStdHepStatus[rtEv.fStdHepN] = 1;  // Final state
        for (int k = 0; k < 4; k++) {
          rtEv.fStdHepX4[rtEv.fStdHepN][k] = 0;
        }
        for (int k = 0; k < 3; k++) {
    ...
    std::cout << std::endl
              << "[INFO]: Wrote Output file: " << fname.c_str() << " with "
