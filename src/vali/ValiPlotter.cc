...
VarToPlot1D::VarToPlot1D()
    : LegendTitle(""), LegTextScale(1), LegX1(0), LegY1(0), LegX2(0), LegY2(0) {
  Reset();
}
VarToPlot2D::VarToPlot2D()
    : NYBins(0),
      YBinMin(0),
      YBinMax(0),
      YAxisTitle(""),
...
VarToPlot1D_2Dial::VarToPlot1D_2Dial()
    : LegendTitle(""),
      LegTextScale(0.8),
      LegX1(0),
      LegY1(0),
...
void PlotVar1D(VarToPlot1D &v, PlotData &pd) {
  ...
  TH1D *h_dialm2 = strTH1<TH1D>(
      h_m2_name, v.DialPrettyName + " = " + v.DialValm2 + " " + v.DialUnits,
      v.NXBins, v.XBinMin, v.XBinMax);
  TH1D *h_dial0 = strTH1<TH1D>(
      h_0_name, v.DialPrettyName + " = " + v.DialVal0 + " " + v.DialUnits,
      v.NXBins, v.XBinMin, v.XBinMax);
  TH1D *h_dial2 = strTH1<TH1D>(
      h_2_name, v.DialPrettyName + " = " + v.DialVal2 + " " + v.DialUnits,
      v.NXBins, v.XBinMin, v.XBinMax);

  std::string h_0_wm2_name = v.XVariableName + "_dial0_weightm2";
  std::string h_0_w2_name = v.XVariableName + "_dial0_weight2";
  TH1D *h_dial0_weight_m2 = strTH1<TH1D>(
      h_0_wm2_name, v.DialPrettyName + " = " + v.DialVal0 + " " + v.DialUnits +
                        ", Reweighted to " + v.DialValm2 + " " + v.DialUnits,
      v.NXBins, v.XBinMin, v.XBinMax);
  TH1D *h_dial0_weight_2 = strTH1<TH1D>(
      h_0_w2_name, v.DialPrettyName + " = " + v.DialVal0 + " " + v.DialUnits +
  ...
  Draw(pd.tree_dial0, v.PlotString + " >> " + h_0_wm2_name,
       std::string("weight_") + v.DialName + "_0_to_minus2*(EvtWght*(" +
           v.SelectionString + "))/" + pd.nent_dial0 + ".0",
       "GOFF");

  Draw(pd.tree_dial0, v.PlotString + " >> " + h_0_w2_name,
       std::string("weight_") + v.DialName + "_0_to_2*(EvtWght*(" +
           v.SelectionString + "))/" + pd.nent_dial0 + ".0",
       "GOFF");
  ...
  TPad *ratioP =
      new TPad((v.XVariableName + "_ratio").c_str(), "", 0, 0, 1, yFrac);
  ...
  if (h_dial_m2_rat->GetMaximum() > h_dial_2_rat->GetMaximum()) {
    ...
    EnlargeAx(h_dial_m2_rat->GetXaxis(), 1.5 / (1.0 - yFrac), 0.9,
              1.6 / (1.0 - yFrac), 1.0);
    EnlargeAx(h_dial_m2_rat->GetYaxis(), 1.5 / (1.0 - yFrac), 0.35,
              1.3 / (1.0 - yFrac), 1.0);
    ...
    EnlargeAx(h_dial_2_rat->GetXaxis(), 1.5 / (1.0 - yFrac), 0.9,
              1.6 / (1.0 - yFrac), 1.0);
    EnlargeAx(h_dial_2_rat->GetYaxis(), 1.5 / (1.0 - yFrac), 0.35,
              1.3 / (1.0 - yFrac), 1.0);
...
void PlotVar1D_2Dial(VarToPlot1D_2Dial &v, PlotData &pd) {
  ...
  TH1D *h_dialm2 =
      strTH1<TH1D>(h_m2_name, v.Dial1PrettyName + " = " + v.Dial1Valm2 + " " +
                                  v.Dial1Units + ", " + v.Dial2PrettyName +
                                  " = " + v.Dial2Valm2 + " " + v.Dial2Units,
                   v.NXBins, v.XBinMin, v.XBinMax);
  TH1D *h_dial0 =
      strTH1<TH1D>(h_0_name, v.Dial1PrettyName + " = " + v.Dial1Val0 + " " +
                                 v.Dial1Units + ", " + v.Dial2PrettyName +
                                 " = " + v.Dial2Val0 + " " + v.Dial2Units,
                   v.NXBins, v.XBinMin, v.XBinMax);
  TH1D *h_dial2 =
      strTH1<TH1D>(h_2_name, v.Dial1PrettyName + " = " + v.Dial1Val2 + " " +
                                 v.Dial1Units + ", " + v.Dial2PrettyName +
                                 " = " + v.Dial2Val2 + " " + v.Dial2Units,
                   v.NXBins, v.XBinMin, v.XBinMax);
  ...
  TH1D *h_dial0_weight_m2 = strTH1<TH1D>(
      h_0_wm2_name,
      v.Dial1PrettyName + " = " + v.Dial1Val0 + " " + v.Dial1Units +
          " #Rightarrow " + v.Dial1Valm2 + " " + v.Dial1Units + ", " +
          v.Dial2PrettyName + " = " + v.Dial2Val0 + " " + v.Dial2Units +
          " #Rightarrow " + v.Dial2Valm2 + " " + v.Dial2Units,
      v.NXBins, v.XBinMin, v.XBinMax);
  TH1D *h_dial0_weight_2 = strTH1<TH1D>(
      h_0_w2_name,
      v.Dial1PrettyName + " = " + v.Dial1Val0 + " " + v.Dial1Units +
          ", #Rightarrow " + v.Dial1Val2 + " " + v.Dial1Units + ", " +
          v.Dial2PrettyName + " = " + v.Dial2Val0 + " " + v.Dial2Units +
  ...
  Draw(pd.tree_dialm2, v.PlotString + " >> " + h_m2_name,
       ...
       "GOFF");
  std::cout << "Plotting: \""
            << (std::string("(EvtWght*(" + v.SelectionString + "))/") +
                pd.nent_dialm2 + ".0")
            << "\"" << std::endl;
  ...
  Draw(pd.tree_dial0, v.PlotString + " >> " + h_0_wm2_name,
       ...
       "GOFF");
  std::cout << "Plotting: \""
            << (std::string("weight_") + v.Dial1Name + "_0_to_minus2_" +
                v.Dial2Name + "_0_to_minus2*(EvtWght*(" + v.SelectionString +
                "))/" + pd.nent_dial0 + ".0")
            << "\"" << std::endl;
  ...
  Draw(pd.tree_dial0, v.PlotString + " >> " + h_0_w2_name,
       ...
       "GOFF");
  std::cout << "Plotting: \""
            << (std::string("weight_") + v.Dial1Name + "_0_to_2_" +
                v.Dial2Name + "_0_to_2*(EvtWght*(" + v.SelectionString + "))/" +
                pd.nent_dial0 + ".0")
            << "\"" << std::endl;
  ...
  TH1D *h_dial_m2_rat = static_cast<TH1D *>(h_dial0_weight_m2->Clone(
      (std::string(h_dialm2->GetName()) + "_rat").c_str()));
  TH1D *h_dial_2_rat = static_cast<TH1D *>(h_dial0_weight_2->Clone(
      (std::string(h_dial2->GetName()) + "_rat").c_str()));
  h_dial_m2_rat->Divide(h_dial0_weight_m2, h_dialm2, 1, 1, "b");
  h_dial_2_rat->Divide(h_dial0_weight_2, h_dial2, 1, 1, "b");
...
void PlotVar2D(VarToPlot2D &v, PlotData &pd) {
  ...
  TH2D *h_dialm2 = strTH2<TH2D>(
      h_m2_name, v.DialPrettyName + " = " + v.DialValm2 + " " + v.DialUnits,
      v.NXBins, v.XBinMin, v.XBinMax, v.NYBins, v.YBinMin, v.YBinMax);
  TH2D *h_dial2 = strTH2<TH2D>(
      h_2_name, v.DialPrettyName + " = " + v.DialVal2 + " " + v.DialUnits,
      v.NXBins, v.XBinMin, v.XBinMax, v.NYBins, v.YBinMin, v.YBinMax);

  std::string h_0_wm2_name =
      v.XVariableName + v.YVariableName + "_dial0_weightm2";
  std::string h_0_w2_name =
      v.XVariableName + v.YVariableName + "_dial0_weight2";
  TH2D *h_dial0_weight_m2 = strTH2<TH2D>(
      h_0_wm2_name, v.DialPrettyName + " = " + v.DialVal0 + " " + v.DialUnits +
                        ", Reweighted to " + v.DialValm2 + " " + v.DialUnits,
      v.NXBins, v.XBinMin, v.XBinMax, v.NYBins, v.YBinMin, v.YBinMax);
  TH2D *h_dial0_weight_2 = strTH2<TH2D>(
      h_0_w2_name, v.DialPrettyName + " = " + v.DialVal0 + " " + v.DialUnits +
                       ", Reweighted to " + v.DialVal2 + " " + v.DialUnits,
      v.NXBins, v.XBinMin, v.XBinMax, v.NYBins, v.YBinMin, v.YBinMax);
  ...
  TH2D *h_dialm2_over_dial0_wm2 = strTH2<TH2D>(
      h_m2_name + "__" + h_0_wm2_name,
      v.DialPrettyName + " = " + v.DialValm2 + " " + v.DialUnits, v.NXBins,
      v.XBinMin, v.XBinMax, v.NYBins, v.YBinMin, v.YBinMax);
  TH2D *h_dial2_over_dial0_w2 = strTH2<TH2D>(
      h_2_name + "__" + h_0_w2_name,
      v.DialPrettyName + " = " + v.DialValm2 + " " + v.DialUnits, v.NXBins,
      v.XBinMin, v.XBinMax, v.NYBins, v.YBinMin, v.YBinMax);
  ...
  SetTitle(h_dialm2->GetZaxis(),
           "#frac{d^{2}#sigma}{d" + v.XVariablePrettyName +
  ...
  SetTitle(h_dial0_weight_m2->GetZaxis(),
           "#frac{d^{2}#sigma}{d" + v.XVariablePrettyName +
               v.YVariablePrettyName + "} (cm^{2} nucleon^{-1} GeV^{-1})");
  ...
  SetTitle(h_dialm2_over_dial0_wm2->GetZaxis(),
           "#frac{" + v.DialPrettyName + " = " + v.DialVal2 + "}{" +
               v.DialPrettyName + " = " + v.DialVal0 + " " + v.DialUnits +
               ", Reweighted to " + v.DialValm2 + " " + v.DialUnits + "}");
  ...
  SetTitle(h_dial0_weight_2->GetZaxis(),
           "#frac{d^{2}#sigma}{d" + v.XVariablePrettyName +
               v.YVariablePrettyName + "} (cm^{2} nucleon^{-1} GeV^{-1})");
  ...
  SetTitle(h_dial2_over_dial0_w2->GetZaxis(),
           "#frac{" + v.DialPrettyName + " = " + v.DialVal2 + "}{" +
               v.DialPrettyName + " = " + v.DialVal0 + " " + v.DialUnits +
               ", Reweighted to " + v.DialVal2 + " " + v.DialUnits + "}");
...
void PlotVar2D_2Dial(VarToPlot2D_2Dial &v, PlotData &pd) {
  ...
  //   + v.DialVal2 + " " + v.DialUnits, v.NXBins, v.XBinMin, v.XBinMax,
  //   v.NYBins,
  //   v.YBinMin, v.YBinMax);
  ...
  // TH2D* h_dialm2_over_dial0_wm2 = strTH2<TH2D>(h_m2_name + "__" +
  // h_0_wm2_name,
  //   v.DialPrettyName + " = " + v.DialValm2 + " " + v.DialUnits, v.NXBins,
  //   v.XBinMin, v.XBinMax, v.NYBins, v.YBinMin, v.YBinMax);
  ...
  // SetTitle(h_dialm2->GetXaxis(),v.XAxisTitle);
  // SetTitle(h_dialm2->GetYaxis(),v.YAxisTitle);
  // SetTitle(h_dialm2->GetZaxis(),"#frac{d^{2}#sigma}{d" +
  // v.XVariablePrettyName
  //   + v.YVariablePrettyName + "} (cm^{2} nucleon^{-1} GeV^{-1})");
  // EnlargeAx(h_dialm2->GetXaxis(),1.25,1.2,1.0);
  // EnlargeAx(h_dialm2->GetYaxis(),1.25,1.2,1.0);
  // EnlargeAx(h_dialm2->GetZaxis(),1.0,1.2,1.0);
  ...
  // = "
  //   + v.DialVal2+"}{"+v.DialPrettyName + " = " + v.DialVal0 + " " +
  //   v.DialUnits + ", Reweighted to " + v.DialValm2 + " " + v.DialUnits+"}");
  // EnlargeAx(h_dialm2_over_dial0_wm2->GetXaxis(),1.25,1.2,1.0);
  // EnlargeAx(h_dialm2_over_dial0_wm2->GetYaxis(),1.25,1.2,1.0);
  // EnlargeAx(h_dialm2_over_dial0_wm2->GetZaxis(),1.0,1.35,1.0);
  // h_dialm2_over_dial0_wm2->Draw("COLZ");
...
void PlotValiFitResults(TH1D *FDDistrib, TH1D *MCPreFit, TH1D *MCPostFit,
                        TH1D *MCTrueReweight, std::string XAxisName,
                        std::string YAxisName, std::string LegendTitle,
