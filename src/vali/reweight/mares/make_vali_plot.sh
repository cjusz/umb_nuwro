...
DIALNAME="kNuwro_MaRES"
DIALPRETTYNAME="M_{A}^{RES}"
NEVS="500000"

echo "[INFO]: Using ${PARAM_BASE}, specialised with ${FLUX_HIST}, and ${TARGET_PARAMS}, generating ${NEVS} events to test dial: ${DIALNAME}"

cat ${PARAM_BASE} | sed "s/number_of_events   = 100000/number_of_events = ${NEVS}/g" | sed "s/pion_axial_mass =.*$/pion_axial_mass=0.940/g" | sed "s:#@beam/ND280.txt:@${FLUX_HIST}:g" | sed "s:@target/C.txt:@${TARGET_PARAMS}:g" | sed -e "s/dyn_\([^r].\+_[nc]c\) =1/dyn_\1=0/g" | sed -e "s/dyn_res_nc =1/dyn_res_nc=0/g" > dial_0_params.txt
cat ${PARAM_BASE} | sed "s/number_of_events   = 100000/number_of_events = ${NEVS}/g" | sed "s/pion_axial_mass =.*$/pion_axial_mass=1.140/g" | sed "s:#@beam/ND280.txt:@${FLUX_HIST}:g" | sed "s:@target/C.txt:@${TARGET_PARAMS}:g" | sed -e "s/dyn_\([^r].\+_[nc]c\) =1/dyn_\1=0/g" | sed -e "s/dyn_res_nc =1/dyn_res_nc=0/g" > dial_2_params.txt
cat ${PARAM_BASE} | sed "s/number_of_events   = 100000/number_of_events = ${NEVS}/g" | sed "s/pion_axial_mass =.*$/pion_axial_mass=0.740/g" | sed "s:#@beam/ND280.txt:@${FLUX_HIST}:g" | sed "s:@target/C.txt:@${TARGET_PARAMS}:g" | sed -e "s/dyn_\([^r].\+_[nc]c\) =1/dyn_\1=0/g" | sed -e "s/dyn_res_nc =1/dyn_res_nc=0/g" > dial_m2_params.txt
