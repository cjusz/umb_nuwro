...
DIALNAME="kNuwro_SPPDISBkgScale"
...
if grep --quiet "SPPDISBkgScale" ${PARAM_BASE}; then

  cat ${PARAM_BASE} \
  | sed "s/number_of_events   = 100000/number_of_events = ${NEVS}/g" \
  | sed "s/pion_SPPDISBkgScale =.*$/pion_SPPDISBkgScale=${DIALVAL0}/g" \
  | sed "s:#@beam/ND280.txt:@${FLUX_HIST}:g" \
  | sed "s:@target/C.txt:@${TARGET_PARAMS}:g" \
  | sed -e "s/dyn_\([^r].\+_[nc]c\) =1/dyn_\1=0/g" \
  | sed -e "s/dyn_res_nc =1/dyn_res_nc=0/g" > dial_0_params.txt
  cat ${PARAM_BASE} \
  | sed "s/number_of_events   = 100000/number_of_events = ${NEVS}/g" \
  | sed "s/pion_SPPDISBkgScale =.*$/pion_SPPDISBkgScale=${DIALVAL2}/g" \
  | sed "s:#@beam/ND280.txt:@${FLUX_HIST}:g" \
  | sed "s:@target/C.txt:@${TARGET_PARAMS}:g" \
  | sed -e "s/dyn_\([^r].\+_[nc]c\) =1/dyn_\1=0/g" \
  ...
  cat ${PARAM_BASE} \
  | sed "s/number_of_events   = 100000/number_of_events = ${NEVS}/g" \
  | sed "s/pion_SPPDISBkgScale =.*$/pion_SPPDISBkgScale=${DIALVALM2}/g" \
  | sed "s:#@beam/ND280.txt:@${FLUX_HIST}:g" \
  | sed "s:@target/C.txt:@${TARGET_PARAMS}:g" \
  | sed -e "s/dyn_\([^r].\+_[nc]c\) =1/dyn_\1=0/g" \
  ...
  echo "pion_SPPDISBkgScale=${DIALVAL0}" >> dial_0_params.txt

  echo "pion_SPPDISBkgScale=${DIALVAL2}" >> dial_2_params.txt

  echo "pion_SPPDISBkgScale=${DIALVALM2}" >> dial_m2_params.txt
