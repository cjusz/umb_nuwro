...
////////////////RES region is defined in the hadronic mass from 1080 to theparameter res_dis_cut introduced in params.txt
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////// The main idea behind is that DIS contribution simulates non-resonantpart
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/////////// The functions alfa and beta are a priori arbitrary but they have tosatisfy the conditions
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////Channels are labelled by 4 integers corresponding to: nu/anucc/nc   proton/neutron    pi+/pi0/pi-
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////
{
  ...
  double m =
      lepton_mass(abs(lepton),
                  current); // mass of the produced lepton (see pgd header file)
  double m2 = m * m;
  ...
  double bkgr = p.SPPBkgScale; // Ranges from -1 to 1 that
                               // increases amount of nonresonant background
  ...
  // threshold energy for pion production
    ...
    double nu =
        numin +
        (numax - numin) * z * z; // enhance low energy transfers are preferred

    double przedzial = (numax - numin) * (Wmax - 1080) * 2 *
                       z; // but compesated by this jakobian
    ...
    vec hadrspeed =
        momtran / sqrt(W2 + q * q); // parameter of boost to hadronic rest frame
    ...
    if (lepton >
        0) // the first part of the translation of the event into "spp language"
      j = 0;
    else {
      j = 1;
    }
    ...
    int finalcharge =
        charge(nukleon) +
        (1 - k) *
            (1 - 2 * j); // total electric charge of the pion-nucleon system

    if (W < 1210 || fromdis == 0) // PYTHIA does not work in this region and
                                  // special treatment is required
    {
      ...
      double dis2 = fromdis * spp2 *
                    betadis(j, k, l, 2, W, bkgr); // can be made simpler !!!
#else
      double dis0 = fromdis * spp0 * alfadis(j, k, l, 0, W);
      double dis1 = fromdis * spp1 * alfadis(j, k, l, 1, W);
      double dis2 = fromdis * spp2 *
                    alfadis(j, k, l, 2, W); // can be made simpler !!!
#endif
      ...
      if (finalcharge == 2) {
        delta0 = cr_sec_delta(FFset, delta_axial_mass, delta_C5A, E, W, nu,
                              lepton, nukleon, 2212, 211, current) *
                 adel0;
        delta1 = delta2 = 0;
      }
      ...
      if (finalcharge == 1) {
        delta0 = cr_sec_delta(FFset, delta_axial_mass, delta_C5A, E, W, nu,
                              lepton, nukleon, 2112, 211, current) *
                 adel0;
        delta1 = cr_sec_delta(FFset, delta_axial_mass, delta_C5A, E, W, nu,
                              lepton, nukleon, 2212, 111, current) *
                 adel1;
        delta2 = 0;
      }
      ...
      if (finalcharge == 0) {
        delta1 = cr_sec_delta(FFset, delta_axial_mass, delta_C5A, E, W, nu,
                              lepton, nukleon, 2112, 111, current) *
                 adel1;
        delta2 = cr_sec_delta(FFset, delta_axial_mass, delta_C5A, E, W, nu,
                              lepton, nukleon, 2212, -211, current) *
                 adel2;
        delta0 = 0;
      }
      ...
      int nukleon2 = nukleon_out_(W, lepton, nukleon, pion,
                                  current); // which nucleon in the final state

      vect finnuk, finpion;

      kin2part(W, nukleon2, pion, finnuk,
               finpion); // produces 4-momenta of final pair: nucleon + pion
    ...
    else // the algorithm starts from the production of PYTHIA event
    {
      ...
      pythia71->SetMSTU(
          20, 1); // advirsory warning for unphysical flavour switch off
      pythia71->SetMSTU(23, 1); // It sets counter of errors at 0
      pythia71->SetMSTU(26, 0); // no warnings printed

      // PARJ(32)(D=1GeV) is, with quark masses added, used to define the
      // minimum allowable enrgy of a colour singlet parton system
      pythia71->SetPARJ(33, 0.1);

      // PARJ(33)-PARJ(34)(D=0.8GeV, 1.5GeV) are, with quark masses added, used
      // to define the remaining energy below which
      // the fragmentation of a parton system is stopped and two final hadrons
      // formed.
      pythia71->SetPARJ(34, 0.5);
      pythia71->SetPARJ(35, 1.0);

      // PARJ(36) (D=2.0GeV) represents the dependence of the mass of final
      // quark pair for defining the stopping point of the
      // fragmentation. Strongly corlated with PARJ(33-35)

      pythia71->SetPARJ(37, 1.); //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1

      // MSTJ(17) (D=2) number of attemps made to find two hadrons that have a
      // combined mass below the cluster mass and thus allow
      ...
      ///////////     c) single kaon production; this causes technical
      /// complications because NPar=5 also in this case
      //////////////////////////////////////////////////////////////////////////////////////////////////////////

      if (NPar == 5 &&
          (pythiaParticle->K[1][3] == 211 || pythiaParticle->K[1][4] == 211 ||
           pythiaParticle->K[1][3] == 111 || pythiaParticle->K[1][4] == 111 ||
           pythiaParticle->K[1][3] == -211 ||
           pythiaParticle->K[1][4] == -211)) // spp condition
        {

        if (pythiaParticle->K[1][3] == 211 ||
            pythiaParticle->K[1][4] == 211) // the second part
        {
          t = 0;
          pion = 211;
        }
...
#ifdef USE_BETADIS
        double dis_spp =
            fromdis * betadis(j, k, l, t, W, bkgr); // dis contribution
#else
        double dis_spp =
            fromdis * alfadis(j, k, l, t, W); // dis contribution
#endif
        ...
        double delta_spp =
            cr_sec_delta(FFset, delta_axial_mass, delta_C5A, E, W, nu, lepton,
        ...
        } else  // deltaevent
        {
         // deltaevent
          ...
          kin2part(
              W, nukleon2, pion, finnuk,
              finpion); // produces 4-momenta of the final pair: nucleon + pion
